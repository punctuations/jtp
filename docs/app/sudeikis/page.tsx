...
export default function Sudeikis() {
  const imageIdHex = "092ad684f90cbe12";
      ...
      <main className="mx-auto w-full max-w-4xl px-6 py-14 font-sans">
        <header className="mb-10">
          ...
          <p className="mt-3 text-base leading-7 text-black/70">
            ...
            <span className="font-mono font-semibold">
              jtp.mattt.space:8443
            </span>{" "}
            serving 7 images. Use this to test your client implementation
            against a real server with valid TLS certificates.
          </p>
        ...
        <section id="server" className="mb-10">
          ...
          <p className="mt-3 text-sm leading-6 text-black/80">
            The server uses a valid Let&apos;s Encrypt certificate, so the
            client can use standard CA roots (no custom{" "}
            <span className="font-mono">--cert</span> needed).
          </p>
        </section>
...
./target/release/client jtp://jtp.mattt.space --batch`}
        ...
        <section id="catalog" className="mb-10">
          ...
          <p className="mt-2 text-sm leading-6 text-black/80">
            The <span className="font-mono">sudeikis.jpg</span> file has
            ImageID:
          </p>
          <ul className="mt-2 list-disc space-y-1 pl-5 text-sm leading-6 text-black/80">
            <li>
              Full (16 hex): <span className="font-mono">{imageIdHex}</span>
            </li>
            <li>
              Short (8 hex): <span className="font-mono">{imageIdShort}</span>
            </li>
          </ul>
        </section>
        ...
        <section id="examples" className="mb-10">
          ...
          <div className="mt-6">
            ...
            <p className="mt-2 text-sm leading-6 text-black/80">
              Send bytes <span className="font-mono">01 00</span> (ReqType=LIST,
              no keep-alive) to receive the catalog of 7 images.
            </p>
            <pre className="mt-3 overflow-x-auto rounded-lg border border-black/10 bg-white p-4 font-mono text-xs leading-5">
...
Downloaded 7 images in 3.2s (2.2 images/sec)`}
            ...
            <p className="mt-3 text-sm leading-6 text-black/80 italic">
              Note: ImageIDs shown above are examples. Actual IDs are computed
              from file contents.
            </p>
          </div>
...
cargo run --release --bin client -- --no-tls --addr 127.0.0.1:8443`}
          ...
          <p className="mt-3 text-sm leading-6 text-black/80">
            ...
            (<span className="font-mono">cert.pem</span>,{" "}
            <span className="font-mono">key.pem</span>). Pass{" "}
            <span className="font-mono">--cert cert.pem</span> to the client.
          </p>
        </section>
        ...
        <footer className="pt-6 text-xs text-black/60">
          ...
          <span>
            Server uptime not guaranteed &mdash; this is a demo endpoint.
          </span>
        </footer>
      </main>
    </div>
