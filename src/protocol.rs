...
pub const REQUEST_CANCEL: u8 = 3; // §8.5
pub const REQUEST_WATCH: u8 = 4; // §8.6
...
pub const RESPONSE_GET_BY_ID: &[u8; 4] = b"JTPD"; // §9.2
pub const RESPONSE_BATCH: &[u8; 4] = b"JTPB";
pub const RESPONSE_LIST_AND_GET: &[u8; 4] = b"JTPG";
pub const RESPONSE_CANCEL: &[u8; 4] = b"JTPC"; // §9.6
pub const RESPONSE_WATCH: &[u8; 4] = b"JTPW"; // §9.7
pub const RESPONSE_ERROR: &[u8; 4] = b"JTPE";

// ── Request flags ─────────────────────────────────────────────────────────────
pub const REQUEST_FLAG_KEEP_ALIVE: u8 = 1 << 0;
/// When set in a LIST request, a one-byte FilterMask follows RequestFlags.
/// Each bit N of FilterMask enables FileType N in the response (§8.1).
pub const REQUEST_FLAG_FILTER: u8 = 1 << 1;
// Bits 2–7 remain reserved and MUST be 0.
pub const REQUEST_FLAGS_RESERVED_MASK: u8 = 0b1111_1100;

// ── File-type filter mask ─────────────────────────────────────────────────────
//
// FilterMask is an 8-bit value where bit N corresponds to FileType code N.
// Use LIST_FILTER_ALL (0xFF) or pass None to include every type.
pub const LIST_FILTER_ALL: u8 = 0xff;
pub const LIST_FILTER_PNG: u8 = 1 << 0; // FileType 0
pub const LIST_FILTER_JPEG: u8 = 1 << 1; // FileType 1
pub const LIST_FILTER_WEBP: u8 = 1 << 2; // FileType 2
pub const LIST_FILTER_BMP: u8 = 1 << 3; // FileType 3
pub const LIST_FILTER_GIF: u8 = 1 << 4; // FileType 4
pub const LIST_FILTER_PDF: u8 = 1 << 5; // FileType 5 — §7.1 new in -02
...
// ── WatchEvent ────────────────────────────────────────────────────────────────
// Broadcast payload for WATCH subscriptions (§9.7).
#[derive(Debug, Clone)]
...
impl ImageCatalog {
    ...
    pub fn from_dir(images_dir: impl Into<PathBuf>, name_contains: Option<&str>) -> Self {
        ...
        for entry in paths.flatten() {
            ...
            // §7.1: PDF is FileType 5. All other unrecognised extensions are
            // skipped — we do not use the Unknown (7) code for files found on
            // disk, only for data received over the wire.
            let file_type: u8 = match ext.to_lowercase().as_str() {
                "png" => 0,
                "jpg" | "jpeg" => 1,
                "webp" => 2,
                "bmp" => 3,
    ...
    pub fn add_image(&mut self, meta: ImageMetadata, verbose: bool) {
        if self.images.contains_key(&meta.id) {
            vlog!(
                verbose,
                "Collision on add_image for ID {:016x} ({}), skipping",
                meta.id,
    ...
    fn rebuild_sorted(&mut self) {
        let mut sorted: Vec<ImageId> = self.images
            .values()
            .map(|m| m.id)
            .collect();
    ...
    /// Return the sorted ID list restricted to the file types enabled in
    /// `filter_mask`.  Each bit N enables FileType N.
    ///
    /// Pass `None` or `LIST_FILTER_ALL` (0xFF) to include every type.
    /// FileTypes 6 and 7 are not represented as named bits and always pass
    /// through unchanged.
    pub fn sorted_ids_filtered(&self, filter_mask: Option<u8>) -> Vec<ImageId> {
        let mask = filter_mask.unwrap_or(LIST_FILTER_ALL);
        if mask == LIST_FILTER_ALL {
            return self.cached_sorted.to_vec();
        }
        self.cached_sorted
            .iter()
            .filter(|id| {
                self.images.get(id).map_or(false, |meta| {
                    let ft = file_type_from_flags(meta.flags);
                    // Types 6 and 7 are not addressable by the 8-bit mask;
...
///
/// §8.1 / §9.1: when `filter_mask` is Some and not LIST_FILTER_ALL, only
/// entries whose FileType bit is set in the mask are included in the response.
pub async fn send_catalog(
    stream: &mut (impl AsyncWriteExt + Unpin),
    catalog: &ImageCatalog,
    filter_mask: Option<u8>
) -> tokio::io::Result<()> {
    let ids = catalog.sorted_ids_filtered(filter_mask);
    ...
    for id in ids.iter().take(count as usize) {
        if let Some(metadata) = catalog.images.get(id) {
            ...
            let name_str: String = metadata.file_name
                .file_name()
                .and_then(|s| s.to_str())
                .unwrap_or("")
                .nfc()
...
/// Buffered (single-syscall) LIST response, optionally filtered.
...
pub async fn send_image_with_options(
    ...
    // §7.1: PDF (5) and BMP (3) attempt Zstd compression (fall back to raw if
    // the ratio is poor).  PNG (0), JPEG (1), WebP (2), and GIF (4) are
    // already compressed formats and are sent as-is.  Unknown (7) also
    // attempts compression.
    let should_try_compress = matches!(file_type, 3 | 5 | 7);
...
pub fn validate_request_flags(flags: u8) -> Result<(), std::io::Error> {
    if (flags & REQUEST_FLAGS_RESERVED_MASK) != 0 {
        return Err(
            std::io::Error::new(std::io::ErrorKind::InvalidData, "reserved request flags are set")
        );
    }
    Ok(())
}
...
/// Write a LIST request.
///
/// §8.1: if `filter_mask` is Some and not LIST_FILTER_ALL, bit 1
/// (REQUEST_FLAG_FILTER) is set in RequestFlags and a one-byte FilterMask
/// follows.  Pass `None` or `Some(LIST_FILTER_ALL)` for the full catalog.
pub async fn write_list_request_buffered(
    ...
    match filter_mask {
        Some(mask) if mask != LIST_FILTER_ALL => {
            // Set the FILTER flag and append the mask byte.
            stream.write_all(&[REQUEST_LIST, req_flags | REQUEST_FLAG_FILTER, mask]).await
        }
        _ => {
            // No filter: two-byte request, FILTER flag clear.
            stream.write_all(&[REQUEST_LIST, req_flags & !REQUEST_FLAG_FILTER]).await
        }
    }
}
...
pub async fn send_cancel_ack(stream: &mut (impl AsyncWriteExt + Unpin)) -> tokio::io::Result<()> {
